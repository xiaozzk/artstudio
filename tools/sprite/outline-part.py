...
* 同参数重跑 → 跳过（不会越描越黑）；
* 参数变了 → 报错，要显式 `--force`；
* 想彻底还原 → 重跑 `python tools/sprite/slice-sheet.py`（切件是确定性的，输出可复现）。
...
## 用法
    ...
    # 单件、临时改参数
    python tools/sprite/outline-part.py head_base --width 2.5 --color "#241a17"

    # 只出对照图（原图 | 描边），不落盘
    python tools/sprite/outline-part.py head_base --preview tmp/head_base_outline.png --width 1.5,2,2.5

    # 体检模式：报告会改多少，不写
    python tools/sprite/outline-part.py --config assets/eva_bone/parts/outline.json --check
"""
