...
| 单次成本 | **别按订阅详情里的 `base_usd_per_flow = $0.03283` 估图片编辑** —— 那是文本 flow 的价。实测：2 次成功调用 + 若干次失败请求，余额从 **$10.00 → $8.799（≈$1.20）**，即**一次图片编辑大约 $0.5~0.6**，失败/被掐断的请求也可能计费。要精确对账看 <https://zenmux.ai/platform> 用量页 |
