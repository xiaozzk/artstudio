...
协议（2026-09-23 起默认 --protocol vertex）：
    * vertex（默认）：POST https://zenmux.ai/api/vertex-ai/v1/publishers/{provider}/models/{model}:predict
      ZenMux 统一生图/编辑入口（OpenAI / 通义 / Flux / Kling / Imagen 等都走这里）。
      ...
      部分 provider 回 gcsUri（签名 URL，工具自动下载）。⚠ background / moderation / SSE 流式
      该协议不支持（background 被忽略，透明件走「纯色底 + tools/sprite/flatbg_cut.py 本地抠底」）。
...
def split_model(model: str) -> tuple:
    """'openai/gpt-image-2' → ('openai', 'gpt-image-2')；没有 '/' 则 provider 为空。"""
    if "/" in model:
        return tuple(model.split("/", 1))
    return "", model


# Vertex 协议下按**模型家族**兼容（新模型照表加一格即可）。
# 能力都来自 ZenMux 官方文档的参数映射表 + 实测：
#   openai 用 imageSize/quality 透传；其余家族走 FAMILY_DEFAULT（parameters.aspectRatio）
FAMILIES = {
    "openai": {
...
def model_family(model: str) -> str:
    """'openai/gpt-image-2' → 'openai'；未收录家族一律 'generic'。"""
...
def post_edit_vertex(session, args, key, prompt: str, images: list, mask_png: bytes | None):
    ...
    透传参数（SDK 的 http_options.extra_body → REST 请求体顶层）：
        OpenAI 模型：imageSize / quality；未收录家族：parameters.aspectRatio（+ sampleImageSize）。
    响应 predictions[]：bytesBase64Encoded（Google 系），部分 provider 回 gcsUri（签名 URL，直接 GET）。
    """
    provider, model = split_model(args.model)
    if not provider:
        die(f"Vertex 协议需要 provider/model 形式的模型名（如 openai/gpt-image-2），收到：{args.model}")
    ...
    if not fam["size_param"]:                                  # 非 openai 家族：aspectRatio 控比例
        ...
        if args.sample_image_size:
            body["sampleImageSize"] = args.sample_image_size   # 1K/2K/4K 透传（部分 provider 支持）
    ...
    for i, p in enumerate(preds):
        if p.get("bytesBase64Encoded"):
            ...
            out.append(p["gcsUri"])                        # 签名 URL，save_outputs 会下载
        else:
...
def cmd_edit(args) -> int:
    ...
    if args.protocol == "vertex":
        ...
        if bg_explicit not in (None, "none"):
            warn("Vertex 协议**不支持 background 参数**（官方映射表标 ❌），已忽略。"
                 "要透明件：prompt 里要纯色底（如 #FF00FF）+ tools/sprite/flatbg_cut.py 本地抠底")
        if args.transport != "json":
    ...
    if args.protocol == "openai" and ("2.5" in args.model) and args.background == "transparent":
        warn("gpt-image-2.5 在 edit 端可能不支持 background=transparent（ZenMux 文档列了该取值，"
             ...
             "透明件用「纯色底 + tools/sprite/flatbg_cut.py 本地抠底」")
...
def inject_solid_bg(prompt: str, color, model: str) -> str | None:
    ...
    * 配套：出图后用 tools/sprite/flatbg_cut.py --bg-color <同色> 抠成透明件
    """
    ...
    info(f"--solid-bg 已注入纯色底指令（{color}）；透明件用 tools/sprite/flatbg_cut.py --bg-color {color} 本地抠底；"
...
def add_edit_args(p: argparse.ArgumentParser) -> None:
    ...
    g2.add_argument("--quality", default="low", choices=("low", "medium", "high", "xhigh", "max", "auto"),
                    help="默认 low（实测 $0.018~0.020/张，随输入图大小浮动）。medium≈$0.04~0.05（估）、"
                         "high≈$0.15~0.18（实测）；xhigh/max 只有 2.5 系列认")
