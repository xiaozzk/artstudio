...
为什么不用模板匹配：本仓库的 `tools/spine/repair_spine/pipelines.py` 自带 Renderer，
...
_PIPE = os.path.join(HERE, "repair_spine")            # 同场景目录下的图集工具
...
def cmd_verify(a) -> int:
    ...
    # --preview 是必填参数：顺手拿它跟重渲染对一次账，确认坐标/几何仍可信（只提示、不拦，与 locate 同口径）
    ref = Image.open(a.preview).convert("RGB")
    before = Scene(a.json, a.images_dir, a.skin).preview_rgb()
    if ref.size == before.size:
        dd = np.abs(np.asarray(before, np.int16) - np.asarray(ref, np.int16))
        print(f"与交付预览图对账：尺寸一致 {ref.size}｜RGB 最大差 {int(dd.max())} 平均差 {dd.mean():.3f}")
    else:
        print(f"[warn] 复现尺寸 {before.size} ≠ 交付预览 {ref.size}，坐标映射不可信")
